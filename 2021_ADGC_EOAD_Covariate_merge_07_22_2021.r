...
# the closest because PC2 and PC4 are scaled differently on the plot. If I
# draw with coord_equal I will be able to confirm visually that these are
...
colnames(NEAREST.HISPANIC) <- paste0("NEAREST_HISPANIC_", colnames(NEAREST.HISPANIC))

NEAREST.NEIGHBOR.WIDE <- cbind.data.frame(NEAREST.NHW, NEAREST.HISPANIC)

NEAREST.NEIGHBOR.WIDE$Corr <- "NULL"

for(i in 1:nrow(NEAREST.NEIGHBOR.WIDE)){
...
dim(PCA)
HAPMAP.ethnicty <- read.table("relationships_w_pops_121708.txt", header = T, stringsAsFactors = F )
head(HAPMAP.ethnicty)
PCA <- PCA[1:12]
colnames(PCA) <- c("FID", "IID", c(paste0("PC", 1:10)))

PCA$COHORT <- "ADGC"
PCA$COHORT <- as.character(HAPMAP.ethnicty$population[match(PCA$IID, HAPMAP.ethnicty$IID)])
PCA <- PCA[c("FID", "IID", c(paste0("PC", 1:10), "COHORT"))]
...
## HISPANIC ##
##############
# Select samples that are HISPANIC
PCA.HISPANIC <- SELECTED.HISPANIC[grepl("Reported_Hispanic|Reported_NHW", SELECTED.HISPANIC$ADGC_COHORT),]
table(PCA.HISPANIC$ADGC_COHORT)
##############################
## Remove Hispanic from NHW ##
##############################
sum(PCA.NHW$KEY %in% PCA.HISPANIC$KEY)
# 359
PCA.NHW <- PCA.NHW[!PCA.NHW$KEY %in% HISPANIC.PC2.PC4$KEY,]

table(PCA.NHW$ADGC_COHORT)
###############################
## PLOT the selected samples ##
###############################
# 
SELECTED.POST.HAPMAP <- p.sd.ADGC.WASHU + geom_point(data = PCA[PCA$KEY %in% PCA.NHW$KEY, c("PC1", "PC2")], aes(col="SELECTED_NHW")) +
  scale_color_manual(values = c(ADGC = 'cyan4', CEU='red', JPT = 'green', Reported_NHW = 'yellow', Reported_Hispanic = 'grey', Reported_Asian = 'violet', Reported_African = 'orange', YRI = "blue", WashU = "pink", SELECTED_NHW = "purple"))

SELECTED.POST.HAPMAP <- SELECTED.POST.HAPMAP + geom_point(data = PCA[PCA$KEY %in% PCA.HISPANIC$KEY, c("PC1", "PC2")], aes(col="SELECTED_HISPANIC")) +
  scale_color_manual(values = c(ADGC = 'cyan4', CEU='red', JPT = 'green', Reported_NHW = 'yellow', Reported_Hispanic = 'grey', Reported_Asian = 'violet', Reported_African = 'orange', YRI = "blue", WashU = "pink", SELECTED_NHW = "purple",  SELECTED_HISPANIC = "black"))


SELECTED.POST.HAPMAP <- SELECTED.POST.HAPMAP + geom_point(data = PCA[PCA$KEY %in% PCA.AA$KEY, c("PC1", "PC2")], aes(col="SELECTED_AA")) +
  scale_color_manual(values = c(ADGC = 'cyan4', CEU='red', JPT = 'green', Reported_NHW = 'yellow', Reported_Hispanic = 'grey', Reported_Asian = 'violet', Reported_African = 'orange', YRI = "blue", SELECTED_NHW = "purple", WashU = "pink", SELECTED_HISPANIC = "black", SELECTED_AA = "skyblue")) 


SELECTED.POST.HAPMAP <- SELECTED.POST.HAPMAP + geom_point(data = PCA[PCA$KEY %in% PCA.ASIAN$KEY, c("PC1", "PC2")], aes(col="SELECTED_ASIAN")) +
  scale_color_manual(values = c(ADGC = 'cyan4', CEU='red', JPT = 'green', Reported_NHW = 'yellow', Reported_Hispanic = 'grey', Reported_Asian = 'violet', Reported_African = 'orange', YRI = "blue", SELECTED_NHW = "purple", SELECTED_HISPANIC = "black", SELECTED_AA = "skyblue", WashU = "pink", SELECTED_ASIAN = "chocolate"))

SELECTED.POST.HAPMAP



ggsave("/40/AD/GWAS_data/Source_Plink/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-HapMap-PCA/ADGC/SELECTED-SAMPLES-FROM-ALL-ETHNICITY-ADGC-WashU.jpg", plot = SELECTED.POST.HAPMAP, device = NULL, scale = 1, width = 12, height = 8, dpi = 600, limitsize = TRUE)
...
PCA.HISPANIC$ADGC_COHORT <- "HISPANIC"
PCA.NHW$ADGC_COHORT <- "NHW"
...
# Select samples that are HISPANIC
ALL.HISPANIC.PC2.PC4 <- HISPANIC.PC2.PC4[grepl("HISPANIC|NHW", HISPANIC.PC2.PC4$ADGC_COHORT),]
...
WashU.GWAS.Pheno$KEY2 <- paste(WashU.GWAS.Pheno$FID, WashU.GWAS.Pheno$IID, sep =":")

library(data.table)
ADGC.WASHU.merged <- as.data.frame(rbindlist(list(ADGC.PHENO, WashU.GWAS.Pheno), fill = T))
ADGC.WASHU.merged$PRESUMED_ETHNICITY <- ADGC.WASHU.merged$ETHNICITY

ADGC.WASHU.merged <- ADGC.WASHU.merged[c("FID", "IID", "AGE_LAST_VISIT", "AGE_AT_ONSET", "STATUS", "APOE", "SEX", "APOE4ANY", "ETHNICITY", "COHORT", "PRESUMED_ETHNICITY", "KEY2")]

ADGC.WASHU.merged$ETHNICITY <- NULL

####################################
## Age-filtered samples after PCA ##
####################################
ADGC.WASHU.merged$ETHNICITY[ADGC.WASHU.merged$KEY2 %in% PCA.HISPANIC$KEY] <- "HISPANIC"
ADGC.WASHU.merged$ETHNICITY[ADGC.WASHU.merged$KEY2 %in% PCA.NHW$KEY] <- "NHW"
ADGC.WASHU.merged$ETHNICITY[ADGC.WASHU.merged$KEY2 %in% PCA.AA$KEY] <- "AFRICAN"
ADGC.WASHU.merged$ETHNICITY[ADGC.WASHU.merged$KEY2 %in% PCA.ASIAN$KEY] <- "ASIAN"

ADGC.WASHU.merged <- ADGC.WASHU.merged[!is.na(ADGC.WASHU.merged$ETHNICITY),]
...
sum(ADGC.WASHU.merged$IID %in% washu.array$IID)
# 1993
ADGC.WASHU.merged$STUDY <- washu.array$array[match(ADGC.WASHU.merged$IID,  washu.array$IID)]
ADGC.WASHU.merged$STUDY[is.na(ADGC.WASHU.merged$STUDY)] <- ADGC.WASHU.merged$COHORT[is.na(ADGC.WASHU.merged$STUDY)]

write.table(ADGC.WASHU.merged, "/40/AD/GWAS_data/Source_Plink/2021_ADGC_EOAD/03_Phenotypes/all_covariates/cleaned_phenotypes/ADGC-WashU-phenotype-after-confirming-all-ethnicities-from-PCA.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
...
## CA <= 70 & CO > 70 ##
########################
WashU.GWAS.Pheno.CA70 <- ADGC.WASHU.merged[((ADGC.WASHU.merged$STATUS == 2| ADGC.WASHU.merged$STATUS == 3) & ADGC.WASHU.merged$AGE_AT_ONSET <= 70 & ADGC.WASHU.merged$AGE_AT_ONSET > 0) ,]
WashU.GWAS.Pheno.CA70 <- WashU.GWAS.Pheno.CA70[!is.na(WashU.GWAS.Pheno.CA70$AGE_AT_ONSET),]
dim(WashU.GWAS.Pheno.CA70)
# 8803    13
WashU.GWAS.Pheno.CO70 <- ADGC.WASHU.merged[(ADGC.WASHU.merged$STATUS == 1 & ADGC.WASHU.merged$AGE_LAST_VISIT > 70),]
WashU.GWAS.Pheno.CO70 <- WashU.GWAS.Pheno.CO70[!is.na(WashU.GWAS.Pheno.CO70$AGE_LAST_VISIT),]
dim(WashU.GWAS.Pheno.CO70)
# 17926    13

All.Pheno.CA.lteq.70.CO.gt.70 <- rbind.data.frame(WashU.GWAS.Pheno.CA70, WashU.GWAS.Pheno.CO70)
# > table(All.Pheno.CA.lteq.70.CO.gt.70$PRESUMED_ETHNICITY)
# AFRICAN              ASIAN           HISPANIC                NHW WASHU_NOT_REPORTED 
# 1052               2375                543              20872               1887 

## Ethnicity based on PCA
# > table(All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY)
# AFRICAN    ASIAN HISPANIC      NHW 
# 1444     2387      428    22470              4 
########################
## CA <= 70 & CO > 70 ##
########################
WashU.GWAS.Pheno.CA65 <- ADGC.WASHU.merged[((ADGC.WASHU.merged$STATUS == 2) & ADGC.WASHU.merged$AGE_AT_ONSET <= 65 & ADGC.WASHU.merged$AGE_AT_ONSET > 0) ,]
WashU.GWAS.Pheno.CA65 <- WashU.GWAS.Pheno.CA65[!is.na(WashU.GWAS.Pheno.CA65$AGE_AT_ONSET),]
dim(WashU.GWAS.Pheno.CA65)
# 4021    13
WashU.GWAS.Pheno.CO80 <- ADGC.WASHU.merged[(ADGC.WASHU.merged$STATUS == 1 & ADGC.WASHU.merged$AGE_LAST_VISIT > 80),]
WashU.GWAS.Pheno.CO80 <- WashU.GWAS.Pheno.CO80[!is.na(WashU.GWAS.Pheno.CO80$AGE_LAST_VISIT),]
dim(WashU.GWAS.Pheno.CO80)
# 17926    13

All.Pheno.CA.lteq.65.CO.gt.80 <- rbind.data.frame(WashU.GWAS.Pheno.CA65, WashU.GWAS.Pheno.CO80)
# > table(All.Pheno.CA.lteq.65.CO.gt.80$PRESUMED_ETHNICITY)
# AFRICAN              ASIAN           HISPANIC                NHW WASHU_NOT_REPORTED 
# 387                599                237               8977                875 

## Ethnicity based on PCA
# > table(All.Pheno.CA.lteq.65.CO.gt.80$ETHNICITY)
# AFRICAN    ASIAN HISPANIC      NHW 
# 536      602      196     9741 

#################################################
## See if there are really mix race in the PCA ##
#################################################
## NHW in ASIAN
cc <- SELECTED.POST.HAPMAP + geom_point(data = PCA.ASIAN[PCA.ASIAN$ADGC_COHORT == "NHW", c("PC1", "PC2")], aes(col="REPORTED_NHW_IN_ASIAN")) +
  scale_color_manual(values = c(ADGC = 'cyan4', CEU='red', JPT = 'green', Reported_NHW = 'yellow', Reported_Hispanic = 'grey', Reported_Asian = 'violet', Reported_African = 'orange', YRI = "blue", SELECTED_NHW = "purple", SELECTED_HISPANIC = "black", SELECTED_AA = "skyblue", WashU = "pink", SELECTED_ASIAN = "chocolate", REPORTED_NHW_IN_ASIAN="white"))

## NHW in AA
cc + geom_point(data = PCA.AA[PCA.AA$ADGC_COHORT == "HISPANIC", c("PC1", "PC2")], aes(col="REPORTED_NHW_IN_AA")) +
  scale_color_manual(values = c(ADGC = 'cyan4', CEU='red', JPT = 'green', Reported_NHW = 'yellow', Reported_Hispanic = 'grey', Reported_Asian = 'violet', Reported_African = 'orange', YRI = "blue", SELECTED_NHW = "purple", SELECTED_HISPANIC = "black", SELECTED_AA = "skyblue", WashU = "pink", SELECTED_ASIAN = "chocolate", REPORTED_NHW_IN_ASIAN="white", REPORTED_NHW_IN_AA="white"))


#######################################
## Write files to subset plink files ##
#######################################
## CA <= 70 & CO > 70 ##
########################
All.Pheno.CA.lteq.65.CO.gt.80 <- as.data.frame(All.Pheno.CA.lteq.70.CO.gt.70)
write.table(All.Pheno.CA.lteq.70.CO.gt.70[1:2][All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "NHW",], "/40/AD/GWAS_data/Source_Plink/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/NHW.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
write.table(All.Pheno.CA.lteq.70.CO.gt.70[1:2][All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "HISPANIC",], "/40/AD/GWAS_data/Source_Plink/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/HISPANIC.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
write.table(All.Pheno.CA.lteq.70.CO.gt.70[1:2][All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "AFRICAN",], "/40/AD/GWAS_data/Source_Plink/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/AFRICAN.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
write.table(All.Pheno.CA.lteq.70.CO.gt.70[1:2][All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "ASIAN",], "/40/AD/GWAS_data/Source_Plink/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/ASIAN.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)

## Pheno
gg.NHW.70.70 <- All.Pheno.CA.lteq.70.CO.gt.70[All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "NHW",]
gg.NHW.70.70$STUDY <- gsub("_Hispanic$|_AA$|_Asian$", "", gg.NHW.70.70$STUDY)
write.table(gg.NHW.70.70, "/40/AD/GWAS_data/02_processed/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/PHENO.NHW.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)

gg.HISPANIC.70.70 <- All.Pheno.CA.lteq.70.CO.gt.70[All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "HISPANIC",]
gg.HISPANIC.70.70$STUDY <- gsub("_Hispanic$|_AA$|_Asian$", "", gg.HISPANIC.70.70$STUDY)
write.table(gg.HISPANIC.70.70, "/40/AD/GWAS_data/02_processed/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/PHENO.HISPANIC.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)

gg.AFRICAN.70.70 <- All.Pheno.CA.lteq.70.CO.gt.70[All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "AFRICAN",]
gg.AFRICAN.70.70$STUDY <- gsub("_Hispanic$|_AA$|_Asian$", "", gg.AFRICAN.70.70$STUDY)
write.table(gg.AFRICAN.70.70, "/40/AD/GWAS_data/02_processed/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/PHENO.AFRICAN.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)

gg.ASIAN.70.70 <- All.Pheno.CA.lteq.70.CO.gt.70[All.Pheno.CA.lteq.70.CO.gt.70$ETHNICITY == "ASIAN",]
gg.ASIAN.70.70$STUDY <- gsub("_Hispanic$|_AA$|_Asian$", "", gg.ASIAN.70.70$STUDY)
write.table(gg.ASIAN.70.70, "/40/AD/GWAS_data/02_processed/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/PHENO.ASIAN.confirmed.with.PCA.CA.lteq.70.CO.gt.70.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
########################
## CA <= 65 & CO > 80 ##
########################
All.Pheno.CA.lteq.65.CO.gt.80 <- as.data.frame(All.Pheno.CA.lteq.65.CO.gt.80)
write.table(All.Pheno.CA.lteq.65.CO.gt.80[1:2][All.Pheno.CA.lteq.65.CO.gt.80$ETHNICITY == "NHW",], "/40/AD/GWAS_data/Source_Plink/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET/NHW.confirmed.with.PCA.CA.lteq.65.CO.gt.80.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
...
## IBD ##
#########
setwd("/40/AD/GWAS_data/02_Processed/2021_ADGC_EOAD/02_Processed/01_pre_QC/01-EOAD-preQC/02-Analysis/ADGC-AGE-FILTERED-SUBSET")
## Generate IBD graph in R
### PLOT IBD in R
library(ggplot2)
library(data.table)
IBD<-fread("NHW_confirmed_with_PCA_CA_lteq_65_CO_gt_80_NO_MCI-sex-IBD.genome", head=T)
P<-ggplot(IBD, aes(x=Z0, y=Z1))+ geom_point() + ggtitle("ADGC-WASHU-NHW-9741") + scale_x_continuous(limits=c(0,1)) + scale_y_continuous(limits=c(0,1))
ggsave("ADGC-WASHU-NHW-9741-IBD.jpg", plot = P, device = NULL, scale = 1, width = 16, height = 9, dpi = 300, limitsize = TRUE)



#### Remove related samples
# samples near Z0 0.5 are possibly related, so removing those samples as well
parent_offsping <- IBD[(IBD$Z0 < 0.25 & IBD$Z1 > 0.75), ]
...
relatives.ALL$nIID1 <- with(transform(relatives.ALL, n = 1),  ave(n, IID1, FUN = length))
relatives.ALL$nIID2 <- with(transform(relatives.ALL, n = 1),  ave(n, IID2, FUN = length))
relatives.ALL[, selectedFID := cbind(FID1, FID2)[cbind(.I, max.col(.SD, "first"))],
              .SDcols = nIID1:nIID2]

relatives.ALL[, selectedIID := cbind(IID1, IID2)[cbind(.I, max.col(.SD, "first"))],
              .SDcols = nIID1:nIID2]


write.table(relatives.ALL, "relatives_ALL_NHW.csv", sep ="\t", col.names = T, quote = F)
to.remove <- relatives.ALL[,c("selectedFID", "selectedIID")]
to.remove <- to.remove[-which(duplicated(to.remove)), ]
dim(to.remove)
# 662  2
write.table(to.remove, "relatives_ALL_to_remove_NHW.txt", sep ="\t", col.names = T, quote = F, row.names = F)

##############################
## Merge PCA with Phenotype ##
##############################
PCA.analysis <- read.table("NHW_confirmed_with_PCA_CA_lteq_65_CO_gt_80_NO_MCI-sex-CLEAN1-PCAS.eigenvec", header = T, stringsAsFactors = F)
PCA.analysis$KEY.PCA <- paste(PCA.analysis$FID, PCA.analysis$IID, sep = ":")
PHENO.NHW <- read.table("PHENO.NHW.confirmed.with.PCA.CA.lteq.65.CO.gt.80.NO.MCI.csv", header = T, stringsAsFactors = F)
...
colnames(PHENO.PCA.STUDY)[23:64] <- gsub("STUDY", "", colnames(PHENO.PCA.STUDY)[23:64])
