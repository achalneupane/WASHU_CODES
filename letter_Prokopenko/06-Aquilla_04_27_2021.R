setwd("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files4")

FAM <- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1.fam", sep =" ")
dim(FAM)
FAM$V2 <- as.character(FAM$V2)


## fernandezv@virtual-workstation1:/gscmnt/gc2645/wgs/WXS_Aquilla/03-FINAL/VCFs/JOINT_CALL_7983$ 
# scp ID_LIST_7983_for_VCF.txt achal@fenix.psych.wucon.wustl.edu:///100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files
PROJECTinfo <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/ID_LIST_7983_for_VCF.txt", header = F, sep = "\t")
...
FAM <- cbind(FAM, PHENO[match(FAM$ID_PR_FAM, PHENO$ID_PR_PHENO),])


sum(FAM$V2 == FAM$`Aquilla vcfID`)
sum(FAM$V2 == FAM$SM)

FAM_FINAL <- cbind.data.frame(FID=as.character(FAM$V1), IID=FAM$V2,
                   PID=FAM$PID, MID=FAM$MID,
                   SEX=FAM$Sex, STATUS=FAM$STATUS,
                   APOE=FAM$APOE, AAO=FAM$AAO,
                   ALA=FAM$ALA, AGE=FAM$AGE,
                   PR= as.character(FAM$PR_AN),
                   AuroraEthnicity=as.character(FAM$`ETHNICITY - Aurora`))
...
setwd("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files")
write.table(df, "FASe_3894_Phenoscope.txt", sep =" ", col.names = T, quote = F, row.names = FALSE)
write.table(PLINK, "PLINK.txt", sep =" ", col.names = T, quote = F, row.names = FALSE)
...
## IBD
library(data.table)
IBD <- fread("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP-IBD.genome")
...
write.table(relatives.ALL, "relatives_ALL.txt", sep ="\t", col.names = T, quote = F)


#### Get rid of non_whites
## Merge HAPMAP cohort
PCA <- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP-HAPMAP-MERGED3-for_PCA.eigenvec", header =T, stringsAsFactors=FALSE)
HAPMAP.ethnicty <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/01-Aquilla-preQC/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files2/relationships_w_pops_121708.txt", header = T )
...
PCAs<- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP-PCAS.eigenvec", header=T)

df <- read.table("FASe_3894_Phenoscope.txt",sep =" ", header = T)
dim(df)
...
PCAs$AuroraEthnicity <- as.character(df[match(PCAs$IID, df$IID), "AuroraEthnicity"])
PCAs$AuroraEthnicity[is.na(PCAs$AuroraEthnicity)] <- -9

##plotting:
ggplot(PCAs, aes(x=PC1, y=PC2, color=COHORT, shape = AuroraEthnicity)) + geom_point() + xlab("PC1") + ylab("PC2") + ggtitle("Aquilla 3891") +
  scale_color_manual(values = c('green', 'black', 'red', "blue"))
##plotting:
p <- ggplot(PCAs, aes(x=PC1, y=PC2, color=COHORT)) + geom_point() + xlab("PC1") + ylab("PC2") + ggtitle("Aquilla_FASe 3891") +
  scale_color_manual(values = c('green', 'black', 'red', "blue"))
ggplotly(p)


p <- ggplot(PCAs, aes(x=PC1, y=PC2, color=COHORT, shape = AuroraEthnicity)) + geom_point() + xlab("PC1") + ylab("PC2") + ggtitle("Aquilla_FASe 3891") +
  scale_color_manual(values = c('green', 'black', 'red', "blue"))
ggplotly(p)


source("https://raw.githubusercontent.com/achalneupane/rcodes/b9d2ee172549d81cb3ef8450f2039f316dc27f9b/shiny_interactive_select_PCA.r")
options(max.print=20000)
shinyApp(ui, server)


# Remove duplicates and also keep only NHW
FAM_1951 <- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP.fam", header = F, sep = " ")
dim(FAM_1951)
FAM_1951_non_duplicates <- FAM_1951[!FAM_1951$V2 %in% duplicates$IID2,1:2]
dim(FAM_1951_non_duplicates)

# read NHW samples
NHW <- read.table("NHW_pca_round1.txt", header = T, sep = "\t")
dim(NHW)

non_duplicate_NHW_round1 <- FAM_1951_non_duplicates[FAM_1951_non_duplicates$V2 %in% NHW$IID,]
dim(non_duplicate_NHW_round1)
colnames(non_duplicate_NHW_round1) <- c("FID", "IID")

# remove 62033 and discordant sex
removeSamples <- c("62033", "0_62519_39", "10R_R8_36", "26_FQX_FQX77608", "27_164_85644")
non_duplicate_NHW_round1 <- non_duplicate_NHW_round1[- match (removeSamples, non_duplicate_NHW_round1$IID),]
write.table(non_duplicate_NHW_round1, "1803_non-duplicate_NHW.txt", sep ="\t", col.names = T, quote = F, row.names = F)


## PCA round 2
PCAs<- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-PCAS.eigenvec", header=T)
##plotting:
library(ggplot2)
p <- ggplot(PCAs, aes(x=PC1, y=PC2)) + geom_point() + xlab("PC1") + ylab("PC2") + ggtitle("Aquilla FASe 1608")
ggplotly(p)
...
## PCA round 2 with HAPMAP
PCA <- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-HAPMAP-MERGED3-for_PCA.eigenvec", header =T, stringsAsFactors=FALSE)
HAPMAP.ethnicty <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/06-Aquilla_202101-a/01-Aquilla-preQC/03-PLINK-QC-files2/relationships_w_pops_121708.txt", header = T )
head(HAPMAP.ethnicty)
...
PCA$COHORT <- as.character(PCA$COHORT)
PCA$COHORT[is.na(PCA$COHORT)] <- "FASe"
write.table(PCA, "FASe_3894_WXS_SNPS_INDELS-hwe-geno0.05-mind0.1-HAPMAP-MERGED3-for_PCA.eigenvec-PC1-PC2-COHORT-round2.txt", sep ="\t", col.names = T, quote = F)

df <- read.table("FASe_3894_Phenoscope.txt",sep =" ", header = T)
dim(df)

PCAs<- read.table("FASe_3894_WXS_SNPS_INDELS-hwe-geno0.05-mind0.1-HAPMAP-MERGED3-for_PCA.eigenvec-PC1-PC2-COHORT-round2.txt", header=T)
PCAs$AuroraEthnicity <- as.character(df[match(PCAs$IID, df$IID), "AuroraEthnicity"])
PCAs$AuroraEthnicity[is.na(PCAs$AuroraEthnicity)] <- -9
...
ggplot(PCAs, aes(x=PC1, y=PC2, color=COHORT, shape = AuroraEthnicity)) + geom_point() + xlab("PC1") + ylab("PC2") + ggtitle("Aquilla 3891") +
  scale_color_manual(values = c('green', 'black', 'red', "blue"))
##plotting:
p <- ggplot(PCAs, aes(x=PC1, y=PC2, color=COHORT)) + geom_point() + xlab("PC1") + ylab("PC2") + ggtitle("Aquilla_FASe 1803 with HAPMAP") +
  scale_color_manual(values = c('green', 'black', 'red', "blue"))

# Change the order of color
target <- c("FASe", "JPT", "YRI", "CEU")
PCAs$COHORT <- factor(PCAs$COHORT, levels = target)
PCAs <- PCAs[order(-as.numeric(factor(PCAs$COHORT))),]

ggplot(PCAs) + geom_point(aes(x=PC1, y=PC2, color=COHORT)) + xlab("PC1") + ylab("PC2") + ggtitle("Aquilla_FASe 1803 with HAPMAP") +
...
non_duplicate_NHW_round1$IID <- as.character(non_duplicate_NHW_round1$IID)
demographic_1803 <- FAM_FINAL[match(non_duplicate_NHW_round1$IID, FAM_FINAL$IID),]
...
covars %>%
  mutate(across(starts_with('AGE'), ~replace(., . %in% c(-9, 888, 999), NA))) %>%
  group_by(STATUS, ETHNICITY) %>%
  summarise(MEAN = mean(as.numeric(as.character(AGE_AT_ONSET)), na.rm = TRUE),
            SD = sd(as.numeric(as.character(AGE_AT_ONSET)), na.rm = TRUE),
            RANGE = range(as.numeric(as.character(AGE_AT_ONSET)), na.rm = TRUE))
...
covars %>%
  mutate(across(starts_with('AGE'), ~replace(., . %in% c(-9, 888, 999), NA))) %>%
  group_by(STATUS, ETHNICITY) %>%
  summarise(MEAN = mean(as.numeric(as.character(AGE_LAST_VISIT)), na.rm = TRUE),
            SD = sd(as.numeric(as.character(AGE_LAST_VISIT)), na.rm = TRUE),
            RANGE = range(as.numeric(as.character(AGE_LAST_VISIT)), na.rm = TRUE))
...
## recode bim file for PID
TANZIbim <- read.table("/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-geno-0.02-maf-0.01.fam", header = FALSE)
Phenoscope1803 <- df[match(TANZIbim$V2,df$IID),]
Phenoscope1803$PID <- as.character(Phenoscope1803$PID)
Phenoscope1803$MID <- as.character(Phenoscope1803$MID)
Phenoscope1803$PID[is.na(Phenoscope1803$PID)] <- 0
Phenoscope1803$MID[is.na(Phenoscope1803$MID)] <- 0
Phenoscope1803$PID[Phenoscope1803$PID == "."] <- 0
Phenoscope1803$MID[Phenoscope1803$MID == "."] <- 0

# check the order of file
library(data.table)
PED <- fread("/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/1803_tanzi_ped_recode12.ped", select = c(1:7))
sum(PED$V2 == Phenoscope1803$IID)
Phenoscope1803$PID[Phenoscope1803$PID == Phenoscope1803$MID] <- 0
Phenoscope1803$MID[Phenoscope1803$PID == Phenoscope1803$MID] <- 0

## Add PCA
PCAs <- read.table("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-PCAS.eigenvec", header = TRUE)
Phenoscope1803$IID <- as.character(Phenoscope1803$IID)
colnames(Phenoscope1803)[1:4] <- c("pid", "id", "fid", "mid")
PCAs$IID <- as.character(PCAs$IID)
Phenoscope1803 <- cbind.data.frame(Phenoscope1803, PCAs[match(Phenoscope1803$id, PCAs$IID), c("PC1", "PC2", "PC3", "PC4")])
# sum(tt$id == PED$V2)
# tt$id[!PED$V2== tt$id]
# PED$V2[!tt$id == PED$V2]

Phenoscope1803 <- data.frame(lapply(Phenoscope1803, function(x) {gsub("^-9$", "0", x)}))

## Replace any NAs with 0
Phenoscope1803 <- as.matrix(Phenoscope1803)
Phenoscope1803[is.na(Phenoscope1803)] <- 0

write.table(Phenoscope1803, "/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/FASe_1803_Phenoscope.txt", sep =" ", col.names = T, quote = F, row.names = FALSE)

Phenoscope1803 <- read.table("/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/FASe_1803_Phenoscope.phe", header = T)
Phenoscope1803.txt <- read.table("/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/FASe_1803_Phenoscope.txt", header = T)
sum(table(table(Phenoscope1803$FID))[-1])
# 342 families

table(Phenoscope1803$STATUS)



## After RUNNING FBAT (Single Variant Analysis on Familial data)
# QQ plot
library(lattice)
...
## Read Anotation file
anno_file <- fread("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-snpeff-dbnsfp-ExAC.0.3.GRCh38.vcf.tsv", header = T, sep = "\t")


# ## MAX MAF 0.01

# ## minsize
# # PValueFILE <- "log_FBAT_STATUS_offset_0.15_minsize_1"
# # PValueFILE <- "log_FBAT-v-STATUS_offset_0.15_minsize_1"
# # PValueFILE <- "log_FBAT-v_STATUS_offset_0.15_minsize_3"
# # PValueFILE <- "log_FBAT-v-STATUS_offset_0.15_minsize_10"
# 
# ## rvminsize
# PValueFILE <- "log_FBAT_STATUS_offset_0.15_rvminsize_1"
# PValueFILE <- "log_FBAT_STATUS_offset_0.15_rvminsize_3"
# PValueFILE <- "log_FBAT_STATUS_offset_0.15_rvminsize_4"
# PValueFILE <- "log_FBAT_STATUS_offset_0.15_rvminsize_10" # removes lots of variants


# Fbat
# PValueFILE <- "log_FBAT_STATUS_offset_0.15_minsize_1"
# PValueFILE <- "log_FBAT_STATUS_offset_0.15_minsize_3"
PValueFILE <- "log_FBAT_STATUS_offset_0.15_minsize_4"
# PValueFILE <- "log_FBAT_STATUS_offset_0.15_minsize_10"


LOGISTIC_FBAT <- fread(paste0(PValueFILE,".txt"), header = TRUE, sep =" ", check.names = F)
LOGISTIC_FBAT <- as.data.frame(LOGISTIC_FBAT)
head(LOGISTIC_FBAT)
dim(LOGISTIC_FBAT)
## remove duplicates
LOGISTIC_FBAT <- LOGISTIC_FBAT[!duplicated(LOGISTIC_FBAT$Marker),]

LOGISTIC_FBAT <- LOGISTIC_FBAT[!is.na(LOGISTIC_FBAT$P),]
LOGISTIC_FBAT$P <- as.numeric(LOGISTIC_FBAT$P)
## remove any rows with absolute zeros
sum(LOGISTIC_FBAT$P == 0)
LOGISTIC_FBAT <- LOGISTIC_FBAT[!LOGISTIC_FBAT$P == 0,]
# LOGISTIC_FBAT <- LOGISTIC_FBAT[LOGISTIC_FBAT$afreq < 0.01,]

my.pvalues <- LOGISTIC_FBAT$P
...
LOGISTIC_FBAT <- cbind(LOGISTIC_FBAT,anno_file[match(LOGISTIC_FBAT$Marker, anno_file$key),])
LOGISTIC_FBAT_ANNO <- LOGISTIC_FBAT
...
# LOGISTIC_FBAT
LOGISTIC_FBAT$CHR <- sapply(strsplit(LOGISTIC_FBAT$Marker,":"), `[`, 1)
LOGISTIC_FBAT$BP <- sapply(strsplit(LOGISTIC_FBAT$Marker,":"), `[`, 2)
LOGISTIC_FBAT$SNP <- LOGISTIC_FBAT$Marker

LOGISTIC_FBAT$CHR[LOGISTIC_FBAT$CHR == "X"] <- 23
LOGISTIC_FBAT$CHR[LOGISTIC_FBAT$CHR == "Y"] <- 24
LOGISTIC_FBAT$CHR <- as.numeric(LOGISTIC_FBAT$CHR)
LOGISTIC_FBAT$BP <- as.numeric(LOGISTIC_FBAT$BP)
...
# manhattan(LOGISTIC_FBAT, main = "", ylim=c(0,22), cex = 0.6, cex.axis = 0.9, col = c("blue4", "orange3"), suggestiveline = -log10(1e-06), genomewideline = -log10(1e-08), annotatePval = -log10(1e-08), chrlabs = as.character(c(1:23)))
manhattan(LOGISTIC_FBAT, main = "", ylim=c(0,10), col = c("blue4", "orange3"), suggestiveline = -log10(1e-06), genomewideline = -log10(1e-08), annotateTop = FALSE, annotatePval = 1e-06, chrlabs = as.character(1:23))
dev.off()


write.table(LOGISTIC_FBAT_ANNO, "/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/FBAT_rare_variant_analysis_results.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)


## Match with Tanzi variants
singlevar_Tanzi <- read.table("https://raw.githubusercontent.com/achalneupane/data/master/Rare_variants_showing_association_at_P_5e-04_in_the_NIMH_NIA_ADSP_AD_families.csv", sep =",", header = TRUE)
singlevar_Tanzi$Prokopenko_Nearest_protein.coding.gene <- as.character(singlevar_Tanzi$Prokopenko_Nearest_protein.coding.gene)


# sum(LOGISTIC_FBAT_ANNO$gene %in% singlevar_Tanzi$Nearest.protein.coding.gene)
# LOGISTIC_FBAT_ANNO <- cbind(LOGISTIC_FBAT_ANNO, singlevar_Tanzi[match(LOGISTIC_FBAT_ANNO$gene, singlevar_Tanzi$Nearest.protein.coding.gene), c("Nearest.protein.coding.gene", "VEP.consequence")])
LOGISTIC_FBAT_ANNO <- cbind(LOGISTIC_FBAT_ANNO, singlevar_Tanzi[match(LOGISTIC_FBAT_ANNO$gene, singlevar_Tanzi$Prokopenko_Nearest_protein.coding.gene), ])
REPLICATED_singled_var <- LOGISTIC_FBAT_ANNO[!is.na(LOGISTIC_FBAT_ANNO$Prokopenko_Nearest_protein.coding.gene),]
...
singlevar_Tanzi$Prokopenko_key <- paste(singlevar_Tanzi$Prokopenko_Chromosome, singlevar_Tanzi$Prokopenko_Position, sep =":")
...
q=GRanges(seqnames=LOGISTIC_FBAT_ANNO$`#CHROM`,
           ranges=IRanges(start = LOGISTIC_FBAT_ANNO$POS, end = LOGISTIC_FBAT_ANNO$POS)
)
...
hundreadKB_SV_tmp <- LOGISTIC_FBAT_ANNO [grepl(ALL_overlappingVAR$key[i], LOGISTIC_FBAT_ANNO$key),]  
...
################################
## After RUNNING FBAT (Spatial clustering and single variant analysis on Familial data)
# QQ plot
library(lattice)
source("https://raw.githubusercontent.com/achalneupane/rcodes/master/qqrunif_with_lambda.r")
library (qqman)
...
## rvminsize
PValueFILE <- "Spatial_clustering_window_100_discovery_set_result_rvminsize_2"


LOGISTIC_FBAT <- fread(paste0(PValueFILE,".txt"), header = TRUE, sep =" ", check.names = F)
dim(LOGISTIC_FBAT)
LOGISTIC_FBAT <- as.data.frame(LOGISTIC_FBAT)
LOGISTIC_FBAT$CLUSTER <- LOGISTIC_FBAT$ClID
LOGISTIC_FBAT$END_CIID <- gsub("^.*-", "", LOGISTIC_FBAT$ClID)
LOGISTIC_FBAT$ClID <- sapply(strsplit(LOGISTIC_FBAT$ClID,"-"), `[`, 1)

LOGISTIC_FBAT$REGION <- paste(LOGISTIC_FBAT$ClID, LOGISTIC_FBAT$END_CIID, sep = "-")
...
# The P values of Bonferroni corrected thresholds for suggestive, 5 and 1%
# genome-wide significant levels were 1, 0.05 and 0.01, respectively, divided by
# the number of SNPs used in the GWAS.
# https://gsejournal.biomedcentral.com/articles/10.1186/s12711-017-0295-4
# https://www.biostars.org/p/141785/
# -log10( 1 / number of variants)

# LOGISTIC_FBAT$BP <- p.adjust(LOGISTIC_FBAT$P, method = "bonferroni", n = length(LOGISTIC_FBAT$P))

SUGGESTIVE <- 1/dim(LOGISTIC_FBAT)[1]
GENOMWIDE_SIGNI <- 0.05/dim(LOGISTIC_FBAT)[1]
...
# LOGISTIC_FBAT
LOGISTIC_FBAT$CHR <- sapply(strsplit(LOGISTIC_FBAT$ClID,":"), `[`, 1)
LOGISTIC_FBAT$BP <- sapply(strsplit(LOGISTIC_FBAT$ClID,":"), `[`, 2)
LOGISTIC_FBAT$SNP <- LOGISTIC_FBAT$ClID
...
# https://stackoverflow.com/questions/13774773/check-whether-values-in-one-data-frame-column-exist-in-a-second-data-frame
LOGISTIC_FBAT <- cbind(LOGISTIC_FBAT,anno_file[match(LOGISTIC_FBAT$ClID, anno_file$key),])
LOGISTIC_FBAT_ANNO <- LOGISTIC_FBAT
...
# manhattan(LOGISTIC_FBAT, main = "", ylim=c(0,22), cex = 0.6, cex.axis = 0.9, col = c("blue4", "orange3"), suggestiveline = -log10(1e-06), genomewideline = -log10(1e-08), annotatePval = -log10(1e-08), chrlabs = as.character(c(1:23)))
manhattan(LOGISTIC_FBAT, main = "", ylim=c(0,15), col = c("blue4", "orange3"), suggestiveline = -log10(SUGGESTIVE), genomewideline = -log10(GENOMWIDE_SIGNI), annotateTop = FALSE, annotatePval = 1e-15, chrlabs = as.character(1:24))
dev.off()


write.table(LOGISTIC_FBAT_ANNO, "/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/Spatial_clustering_analysis_results.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
...
LOGISTIC_FBAT_ANNO <- cbind(LOGISTIC_FBAT_ANNO, singlevar_Tanzi[match(LOGISTIC_FBAT_ANNO$gene, singlevar_Tanzi$Nearest.protein.coding.gene), c("Nearest.protein.coding.gene", "VEP.consequence")])
REPLICATED_spatial_var <- LOGISTIC_FBAT_ANNO[!is.na(LOGISTIC_FBAT_ANNO$Nearest.protein.coding.gene),]

colnames(REPLICATED_spatial_var) <- c("Marker", "Allele", "afreq", "fam_size", "S-E(S)", "Var(S)", "Z", "P", "CHROM", "POS", "ID", "REF", "ALT", "key", "consequence", "gene", "type", "region", "TANZI_Nearest.protein.coding.gene", "TANZI_VEP.consequence")
write.table(REPLICATED_spatial_var, "/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/REPLICATED_spatial_variant_genes.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)


sum(LOGISTIC_FBAT_ANNO$P < 0.05)
Nominally_Sig_spatial <- LOGISTIC_FBAT_ANNO[LOGISTIC_FBAT_ANNO$P < 0.05,]
write.table(Nominally_Sig_spatial, "/40/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/FBAT/Nominally_significant_spatial_variant_genes.csv", sep ="\t", col.names = T, quote = F, row.names = FALSE)
...
######################## Epacts results and plots with STATUS only
...
setwd("/100/AD/AD_Seq_Data/05.-Analyses/06-Aquilla_202101/09-Tanzi-replication/01-familial/03-PLINK-QC-files/EPACTS/")
PCA <- read.table("FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_picard_biallelic-hwe-geno0.05-mind0.1_with_STATUS_nonADSP_post_QC2-PCAS.eigenvec", header = T)
PHENO <- read.table("phenotype1.ped", header = FALSE)

sum(PHENO$V2 == PCA$IID)
PHENO <- cbind.data.frame(PHENO, PCA)
PHENO <- PHENO[,colnames(PHENO)[!grepl("FID|IID", colnames(PHENO))]]

write.table(PHENO, "phenotype1_withPCA.ped", sep =" ", row.names = FALSE, col.names = T, quote = F)
...
# LOGISTIC <- fread("Tanzi_1803_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_epacts_single_variant_max-maf_0.01.epacts.gz", header = TRUE, sep ="\t", check.names = F)
LOGISTIC <- fread("Tanzi_1803_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_epacts_single_variant_max-maf_0.01_STATUS_SEX_PC1_PC2_PC3.epacts.gz", header = TRUE, sep ="\t", check.names = F)
...
jpeg("QQ_plot_EPACTs_Complete_stats_epacts_single_variant_max-maf_0.01_STATUS_SEX_PC1_PC2_PC3.jpeg", height = 20, width = 15, units='cm', res = 300)
...
### Max MAF 0.01
# TOP.genes <- read.table("Tanzi_1803_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_epacts_single_variant_max-maf_0.01.epacts.top5000",  comment.char = '&', header = TRUE, sep ="\t", check.names = F)
TOP.genes <- read.table("Tanzi_1803_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_epacts_single_variant_max-maf_0.01_STATUS_SEX_PC1_PC2_PC3.epacts.top5000",  comment.char = '&', header = TRUE, sep ="\t", check.names = F)
...
### Max MAF 0.01
# annotatedGenes <- read.table("Tanzi_1803_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_epacts_single_variant_max-maf_0.01.epacts_EPACTS_variants_to_search_result.txt", header = F, sep = "\t")
annotatedGenes <- read.table("Tanzi_1803_from_Aquilla_after_QC-biallelic_FASe_3894_from_AQUILLA_WXS_SNPS_INDELS_with_no_chr_epacts_single_variant_max-maf_0.01_STATUS_SEX_PC1_PC2_PC3.epacts_EPACTS_variants_to_search_result.txt", header = F, sep = "\t")
...
jpeg("Manhattan_plot_EPACTs_Complete_stats_single_variant_max_maf_0.01_STATUS_SEX_PC1_PC2_PC3.jpeg", height = 20, width = 30, units='cm', res = 300)
...
manhattan(LOGISTIC.t, main = "", ylim=c(0,15), col = c("blue4", "orange3"), suggestiveline = -log10(1e-06), genomewideline = -log10(1e-08), annotateTop = TRUE, annotatePval = -log10(1e-06), chrlabs = as.character(1:23)) 
dev.off()
